#!/bin/bash
#############################################################################
# Pegasus' Linux Administration Tools          container maintenance script #
...
tolog <<EOT
################################################################################
# $PROGRAM_SUITE - #SCRIPT_TITLE V$SHORT_VERSION #
# (c)2017-$CURR_YEAR $MAINTAINER    build $VERSION_BUILD     $EMAIL #
################################################################################

EOT
### APT ########################################################################
create_logline "Updating apt cache" ;				apt-get -qqy update 2>&1 | tolog
create_logline "Updating installed packages" ;		apt-get -qqy --allow-unauthenticated upgrade 2>&1 | tolog
create_logline "Cleaning up obsolete packages"	;	apt-get -qqy autoremove 2>&1 | tolog
create_logline "Purging apt package cache" ;		apt-get -qqy clean 2>&1 | tolog
### GARBAGE ####################################################################
create_logline "Emptying the trash" ;				trash-empty $garbageage 2>&1 | tolog
